...
-- Функция для обхода блокировки изображений
function LoadImageSafe(imageLabel, url)
    local success, result = pcall(function()
        -- Пробуем несколько способов загрузки
        local methods = {
            function() return url end, -- Прямая ссылка
            function() return "https://imageproxy.iris.to/?url=" .. url end, -- Прокси 1
            function() return "https://api.allorigins.win/raw?url=" .. url end, -- Прокси 2
            function() return "https://corsproxy.io/?" .. url end -- Прокси 3
        }
        
        for _, method in ipairs(methods) do
            local proxyUrl = method()
            local imageSuccess = pcall(function()
                imageLabel.Image = proxyUrl
            ...
            if imageSuccess then
                print("✅ Изображение загружено через прокси")
                return true
            end
        end
        return false
    ...
    if not success then
        warn("❌ Не удалось загрузить изображение: " .. tostring(result))
        return false
    end
    return result
end
...
function CreateMenu()
    ...
    loadingText.Text = "🔄 Загрузка изображения..."
    loadingText.TextColor3 = Color3.fromRGB(200, 200, 200)
    loadingText.BackgroundTransparency = 1
    loadingText.TextSize = 14
    loadingText.Parent = imageContainer
    
    -- Загружаем изображение в отдельном потоке
    spawn(function()
        local loaded = LoadImageSafe(imageLabel, IMAGE_URL)
        if loaded then
            loadingText.Visible = false
        else
            loadingText.Text = "❌ Не удалось загрузить изображение"
    ...
    local buttons = {
        ...
        {"🔄 Перезагрузить фото", function() 
            loadingText.Visible = true
            loadingText.Text = "🔄 Перезагрузка изображения..."
            spawn(function()
                LoadImageSafe(imageLabel, IMAGE_URL)
                loadingText.Visible = false
            end)
        end},
